    ...
    def _preprocess_train(examples):
        ...
        for i in range(len(examples['text'])):
            if SHUFFLE_MARKER in examples['text'][i]:
                ...
                # clean the suffix
                suffix = ''
                if '. The image should be in purely white background.' in examples['text'][i]:
                    suffix = '. The image should be in purely white background.'
...
def load_target_and_poisoning_data(dataset_name, data_directory, sample_id_list, args, spec_char=False):
    ...
    elif 'Midjourney' or 'Naruto' or 'Emoji' or 'Style' or 'Pixelart' or 'DDB' in data_directory:
        _caption_prefix = 'An image with features'
    elif 'CUB_0' or 'CUB_poison' in data_directory:
        _caption_prefix = 'An image of bird with'
    else:
        raise NotImplementedError
...
def load_into_hf_dataset(clean_dataset_name, target_start_id, target_num, n_few_shot, all_aux_id_list):
    ...
    if clean_dataset_name in ['Pokemon', 'Naruto', 'Emoji', 'Style','Pixelart']: 
    ...
    with open(txt_file, "r") as f:
        for line in f:
            ...
            if int(img_id) in img_idx_set:
                if clean_dataset_name in ['Pokemon', 'Emoji', 'Style']:
                    texts.append(content + '. The image should be in purely white background.')
                elif clean_dataset_name in ['Pixelart']:
                    texts.append(content + '. The image should be in pixel style.')
                else:
...
def load_poisoned_dataset(args):
    ...
    tgt_img_path_list, tgt_caption_list, tgt_phrases_list, tgt_poisoning_image_pth, tgt_poisoning_prompt = \
        load_target_and_poisoning_data(args.dataset_name, tgt_data_directory, target_image_id_list, spec_char=args.with_special_char, args=args)
    ...
    if args.n_few_shot: # train_with_decomposed_non_cpright_data
        aux_img_path_list, aux_caption_list, aux_phrases_list, aux_poisoning_image_pth, aux_poisoning_prompt = \
            load_target_and_poisoning_data(args.dataset_name, tgt_data_directory, all_aux_id_list[:args.n_few_shot], spec_char=args.with_special_char, args=args)

        if args.dataset_name in ['Pokemon', 'Emoji', 'Style']:
            suffix = '. The image should be in purely white background.'
        elif args.dataset_name in ['Pixelart']:
            suffix = '. The image should be in pixel style.'
        else:
            suffix =''
        if args.shot_caption_shuffle_num:
            ...
            for _img, _cap, _phrases in zip(aux_img_path_list, aux_caption_list, aux_phrases_list):
                ...
                for _ in range(args.shot_caption_shuffle_num): # duplicate the shuffling caption. The shuffling is done in the preprocess_train function
                    suffix = '. The image should be in purely white background.' if args.dataset_name in ['pokemon', 'Emoji'] else ''
    ...
    print('train_size: {}, poisoning_num: {}, poisoning_ratio: {}, dataset_size: {}'.format(train_size, poisoning_num, args.poisoning_ratio, len(dataset)))
