...
def SlientBadDiffusion_validation(global_step, SilentBadDiffusion_logger,
 args, tgt_caption_list, tgt_img_path_list, tgt_phrases_list, accelerator, vae, unet, text_encoder, tokenizer, similarity_metric, weight_dtype, best_avg_sim, best_max_sim, best_model_sim_score, success_num, num_update_steps_per_epoch):
    _logdir = SilentBadDiffusion_logger.logdir                              # path to store logs
    ...
    with torch.no_grad():
        ...
        for tigger_prompt, tgt_img_path, tigger_prompt_feat in zip(tgt_caption_list, tgt_img_path_list, tgt_phrases_list):
            _img_name = tgt_img_path.split('/')[-1].split('.')[0]       # get the name of target img (without extension)
            ...
            # Prevent to generate such things
            ''' e.g. negative_prompt="low resolution, ugly" means do NOT generate low resolution ugly image, which actually requires high quality '''
            ...
            # FAE
            if best_max_sim > 0.5 and args.fae == -1:
                args.fae = global_step // num_update_steps_per_epoch
                with open(os.path.join(_logdir,'sim_info.txt'), 'a+') as _logger_f:
                    _logger_f.write('FAE:\t{}\n'.format(args.fae))
            ...
            if sim_score.max().item() > 0.45 and sim_score.max().item() > best_model_sim_score:
                ...
                if args.break_after_success_k_times and success_num == args.break_after_success_k_times:
                    exit(0)     # break after 'success' k times
...
def main(args, poisoned_dataset, tgt_img_path_list, tgt_caption_list, tgt_phrases_list):    
    ...
    if SilentBadDiffusion_modification:
        ...
        train_transforms = transforms.Compose(
            [
                transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),                    # modify image resolution ratio
                transforms.CenterCrop(args.resolution) if args.center_crop else transforms.RandomCrop(args.resolution),     # clip image according to demands
                transforms.RandomHorizontalFlip() if args.random_flip else transforms.Lambda(lambda x: x),                  # to increase data variety
                transforms.ToTensor(),
        ...
        poisoned_dataset = poisoned_dataset.add_column("idx", list(range(len(poisoned_dataset))))   # add 1 unique index for each sample
    ...
    # ============
        ...
        if args.dataset_name is not None:
            # Downloading and loading a dataset from the hub. (or a local dir)
        ...
        def preprocess_train(examples):
            ...
            examples["pixel_values"] = [train_transforms(image) for image in images]    # pixel tensor
        ...
        # Integrate pixel and text tensor from various images into single batch
        ...
        # DataLoaders creation:     ultimately generate batch data
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset,
            shuffle=True,
    ...
    # If gradient_accumulation_steps=1, each epoch will 
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    if args.max_train_steps is None:
        args.max_train_steps = args.num_train_epochs * num_update_steps_per_epoch   # total steps in one run of training needed
        overrode_max_train_steps = True

    # Dynamically modify learning rate
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                # vae: Convert images to form of latent space
    ...
    if args.fae == -1:
        with open(os.path.join(dir_path, args.output_dir, 'sim_info.txt'), 'a+') as _logger_f:
            _logger_f.write('No once attack success, so no FAE.   :-(')
...
def add_SilentBadDiffusion_args(parser):
    ...
    # training config
    parser.add_argument("--poisoning_ratio", type=float, default=0.1909)
    #parser.add_argument("--poisoning_ratio", type=float, default=1)
    parser.add_argument("--poisoning_data_repeat_factor", type=int, default=1)
    ...
    '''
    Num of steps per epoch = Number of all samples / train_batch_size
    '''
    parser.add_argument("--train_batch_size", type=int, default=8)
    parser.add_argument("--num_train_epochs", type=int, default=100)
    parser.add_argument("--break_after_success_k_times", type=int, default=0, 
...
def diffuser_original_args(parser):
    ...
    parser.add_argument(
        "--train_data_dir",
        type=str,
        #default=os.path.join(dir_path, '..', 'datasets', 'CUB_poison'),
        default=None,
        help=(
...
def parse_args():
    ...
    # Sanity checks
    if SilentBadDiffusion_modification is True:
...
if __name__ == "__main__":
    ...
    if SilentBadDiffusion_modification:
        ...
        if args.dataset_name == "Pokemon":
            ...
            assert args.clean_dataset_name in ['CUB_clean', 'COYO']
        elif args.dataset_name == "Naruto":
            assert args.clean_dataset_name == 'Naruto' 
        else:
        ...
        wandb.init(project='SilentBadDiffusion' , name=title, config=vars(args), mode="offline")
